  ...
  describe 'Initial state' do
    context '#initialize' do
      it 'sets @epdcx_parser to a new instance of EprintsDcXmlParser' do
        expect(described_class.new('test-sn',
                                   'firsttestdepositor').epdcx_parser).to be_an_instance_of(Sword::Parsers::EprintsDcXmlParser)
      end

      it 'sets @hyacinth_adapter to a new instance of HyacinthAdapter' do
        expect(described_class.new('test-sn',
                                   'firsttestdepositor').hyacinth_adapter).to be_an_instance_of(Sword::Adapters::Hyacinth::V2::HyacinthAdapter)
      end
    end
  end
