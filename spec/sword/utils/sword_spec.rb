...
RSpec.describe Sword::Utils::Sword do
  describe '.collection_info_for_service_document' do
    it 'contains expected values' do
      info = described_class.collection_info_for_service_document('test-pq')
  ...
  describe '.pull_credentials' do
    it 'returns array with Basic auth info' do
      ...
      depositor, passwd = described_class.pull_credentials request_double
  ...
  describe '.create_deposit' do
    it 'returns service document content' do
      deposit = described_class.create_deposit('test_user',
  ...
  describe '.service_document_content' do
    context 'with config info' do
      it 'returns service document content' do
        expect(described_class.service_document_content).to include 'sword_version'
      end
    end
  end
  ...
  describe '.service_document_xml' do
    context 'with mime type and package type specified' do
      it 'returns service document content' do
        ...
        expect(described_class.service_document_xml(content)).to include 'a-package-type'
      end
    end
    ...
    context '.without mime type and package type specified' do
      it 'returns service document content' do
        ...
        content[:sword_verbose] = false
        content[:http_host] = 'something'
        expect(described_class.service_document_xml(content)).to include 'Title of Proquest Test'
      end
    end
  end
end
