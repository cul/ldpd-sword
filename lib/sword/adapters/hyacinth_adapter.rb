...
require 'net/http'
module Sword::Adapters::HyacinthAdapter
  ...
  def ingest_asset(parent_pid, document_filepath)
  ...
  def compose_dynamic_field_data
    ...
    encode_genre unless @genre_uri.nil? && @genre_value.nil?
    encode_language unless @language_uri.nil? && @language_value.nil?
    encode_license unless @license_uri.nil?
    encode_names unless (@corporate_names.empty? && @personal_names.empty?)
  ...
  def encode_personal_names
    ...
    if HYACINTH_CONFIG.has_key? :max_number_names && @personal_names.length > HYACINTH_CONFIG[:max_number_names]
  ...
  def prep_name(person)
    if person.full_name_naf_format.nil?
      ...
      value_data = person.full_name_naf_format.to_s
